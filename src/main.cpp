...
unsigned int nStakeMinAge = 8 * 60 * 60; // 8 hours changed from 6
...
const string strMessageMagic = "ToroCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    {
        nSubsidy = 450000 * COIN; //  PREMINE 1 BLOCK
    }
    else if(nHeight < LAUNCH_BLOCK)
    {
        nSubsidy = 0 * COIN; // No reward block
    }
		else if(nHeight < POW_BLOCKS)
    {
        nSubsidy = 20 * COIN;
    }
		else if(nHeight < POW2POS_BLOCKS)
    {
        nSubsidy = 5 * COIN;
    }
    else
    {
        nSubsidy = 0 * COIN; //We end POW to move into MN and POS
    }
...
// miner's coin stake reward based on coin age spent (coin-days) (Old)
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    ...
    if(nHeight < LAUNCH_BLOCK)
    {
        nSubsidy = 0 * COIN; // first 50 blocks we get nothing
    }
		else if(nHeight < POW_BLOCKS )
    {
        nSubsidy = 10 * COIN; // we will slowly increase rewards

    }
		else if(nHeight < POW2POS_BLOCKS )
    {
        nSubsidy = 12 * COIN;  //looking better
    ...
    return nSubsidy + nSuperBlockReward + nFees;
}
...
#ifdef ENABLE_WALLET
// torocoin: attempt to generate suitable proof-of-stake
