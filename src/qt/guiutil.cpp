...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("ToroCoin"))
        return false;
...
{
    // Convert ToroCoin:// to ToroCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ToroCoin://"))
    {
        uri.replace(0, 12, "ToroCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ToroCoin.lnk";
}
...
{
    return GetAutostartDir() / "ToroCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ToroCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ToroCoin-qt [" + tr("command-line options") + "]                     " + "\n";
