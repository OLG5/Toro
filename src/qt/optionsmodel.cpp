...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeToroCoinAmount"))
        settings.setValue("nAnonymizeToroCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeToroCoinAmount = settings.value("nAnonymizeToroCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeToroCoinAmount"))
        SoftSetArg("-anonymizeToroCoinamount", settings.value("nAnonymizeToroCoinAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeToroCoinAmount:
            nAnonymizeToroCoinAmount = value.toInt();
            settings.setValue("nAnonymizeToroCoinAmount", nAnonymizeToroCoinAmount);
            emit anonymizeToroCoinAmountChanged(nAnonymizeToroCoinAmount);
            break;
        default:
            break;
        }
