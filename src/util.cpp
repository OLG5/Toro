...
int nAnonymizeToroCoinAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ToroCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ToroCoin
    // Mac: ~/Library/Application Support/ToroCoin
    // Unix: ~/.ToroCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ToroCoin";
#else
    fs::path pathRet;
...
void createConf()       //Automatic torocoin.conf generation
{
    srand(time(NULL));
    ...
    const char* nodes =  "\nrpcport=42344"
                         ...
                         "\nserver=1"
                         "\nlistenonion=0"
                         "\naddnode=node1.torocoin.net"
                         "\naddnode=node2.torocoin.net"
                         "\naddnode=node3.torocoin.net";
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "torocoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "torocoind.pid"));
